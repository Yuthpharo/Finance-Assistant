...
file_path = 'personal finance data 2.xlsx'  # Ensure this file exists in the same directory
...
def expenses():
    ...
    if date_filter == 'last_month':
        ...
        start_date = current_date - timedelta(days=90)  # Approximate last 3 months
        end_date = current_date
        filtered_data = data[(data['Date / Time'] >= start_date) & (data['Date / Time'] <= end_date)]
    else:
        # Default to all-time data if no filter is selected
        filtered_data = data
...
def budget():
    ...
    if request.method == 'POST':
        ...
        # Add or update a bill
            ...
            bill_due_date = request.form.get('bill_due_date')  # Retain only the due date
...
def insights():
    ...
    if date_filter == 'last_month':
        ...
        filtered_data = data[(data['Date / Time'] >= start_date) & (data['Date / Time'] <= current_date)]
    else:
        filtered_data = data  # Default to all-time data if no filter is selected
    ...
    return render_template(
        ...
        date_filter=date_filter  # Pass the selected filter to the template
    )
...
def advice():
    ...
    # Generate new data for predicting the next 30 days or 1 month
    current_date = pd.to_datetime('today')
    next_month = current_date + pd.DateOffset(months=1)
    predicted_expense = 0

    # Generate prediction for each day or month in the next 30 days
    for i in range(30):  # Simulating predictions for the next 30 days
        new_data = {
            'Category': 'Category_Name',  # You can set dynamic values based on patterns or previous data
            'Sub category': 'Sub_Category_Name',
            'Mode': 'Mode_Type',
            'Income/Expense': 'Expense',  # Use 'Expense' as you're predicting for expenses
        }
    ...
    # Round the predicted expense to 2 decimal places
    ...
    # Calculate financial health for the last 3 months
    ...
    # Identify most expensive category in the last 3 months
    expense_by_category = (
